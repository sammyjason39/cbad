...
export default function LoadingScreen({ files, onDone }) {
  ...
  return (
    <div
      ...
      style={{ background: brand.ink, opacity: fading ? 0 : 1 }}
    >
      ...
      <div className="w-full max-w-2xl flex flex-col items-center gap-8">
        <div className="text-center">
          <h1 className="text-white font-bold tracking-tight" style={{ fontSize: 30, letterSpacing: '-0.02em' }}>
        ...
        <div className="relative flex items-center justify-center" style={{ width: 200, height: 200 }}>
          <svg width="200" height="200" viewBox="0 0 200 200" style={{ transform: 'rotate(-90deg)' }}>
            <circle cx="100" cy="100" r={RADIUS} fill="none"
              stroke="rgba(22,82,240,0.12)" strokeWidth="8" />
            <circle cx="100" cy="100" r={RADIUS} fill="none"
              stroke={brand.blue} strokeWidth="8"
              strokeLinecap="round"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-white font-extrabold" style={{ fontSize: 48, lineHeight: 1, letterSpacing: '-0.03em' }}>
              {pct}
            </span>
            <span className="font-mono text-xs uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.4)' }}>%</span>
          </div>
        </div>
        ...
        <div className="w-full rounded-full overflow-hidden" style={{ height: 3, background: 'rgba(255,255,255,0.08)' }}>
          <div
            ...
            style={{
              width: `${pct}%`,
              background: `linear-gradient(90deg, ${brand.blue}, ${brand.blueSoft})`,
              transition: 'width 0.12s linear',
            }}
          />
        </div>

        <p className="text-sm font-mono text-center uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.35)', minHeight: 20 }}>
          {getStageLabel(pct)}
        </p>

        <div
          ref={logRef}
          className="w-full font-mono text-xs overflow-y-auto"
          style={{
            height: 180,
            borderRadius: 16,
        ...
        >
          {logLines.slice(0, visibleCount).map((line, i) => (
            <div key={i} className="leading-relaxed">
              <span style={{ color: 'rgba(22,82,240,0.5)' }}>[{line.ts}]</span>{' '}
              {line.text}
            </div>
          ))}
          {visibleCount < logLines.length && (
            <span className="animate-pulse" style={{ color: brand.blue }}>▋</span>
          )}
        </div>
      </div>
    </div>
